...
::: warning 鉴权现状
当前所有服务**不验签 JWT**,涉及用户的接口把 `userId` 当普通参数接收(可被伪造)。这是已登记的设计债,整改方案见 [ADR-001 统一鉴权](/platform/adr-001-unified-auth)。本页"鉴权"列标注的是**整改后**的目标状态。
:::
...
- **分页**:`page` 从 **0** 开始(MVP-2 曾因 App 传 `page=1` 导致搜索空结果,已修)。
