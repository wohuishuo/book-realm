...
### 11. v2.2 交互修正:从底部弹窗改成浮动工具条

**结论:划线交互应该发生在正文附近,不是把读者拖到一个大弹窗里。**

真机试用后,底部大 sheet 被判定为“不像阅读 App”。本轮改成:

- 长按段落后,段落进入蓝色选中态;
- 页面下方出现黑色浮动工具条;
- 工具条提供复制、划线、写想法、AI 问书、听当前;
- 写想法时只展开一个小面板,不离开阅读上下文。

同时修复两个数据问题:

- 旧登录态可能没有 `userId`,导致明明已登录却提示不能划线;现在 token 存在时先按演示用户兜底;
- 搜书会重新写 Room 缓存,以前会把已加入书架的 `inShelf` 覆盖成 `false`;现在写入前会保留本地书架状态。

当前已经完成章节正文离线缓存。现在保存的是书架元数据、登录态、字号、阅读进度和章节正文;加入书架时会预缓存前 8 章,在线打开过的章节也会写入 Room。

### 12. v2.2 AI 临时窗口与范围选择

**结论:AI 问书应该短暂接管屏幕,而不是挤在正文里。**

参考微信读书后,本轮把 AI 从底部 bubble 改成黑色临时全屏窗口:

- 顶部显示“AI 问书”和当前章节;
- 中部显示推荐问题和快捷能力;
- 底部保留输入框;
- 摘要结果也进入这个窗口,不再插入正文列表。
...
这还不是最终的“手指拖动精确选字”。Compose 要做微信读书那种字级选区,需要自定义文本布局和手势命中;当前先用段落范围选择保证功能可用。

### 13. v2.2 本地离线阅读

**结论:书架不能只是书名列表,它必须能在断网时继续读。**

本轮新增 `chapter_cache` 表,把章节详情落到 Room:

- 在线打开章节时缓存 `ChapterDetailDto`;
- 加入书架时预缓存前 8 章;
- 网络失败时,Repository 会从本地缓存回退;
- 已缓存章节断网也能打开。
...
**结论:组件库必须立刻服务页面,否则只是另一层空抽象。**

本轮新增:

- `SectionHeader`:页面区块标题;
- `SearchField`:搜索输入与搜索按钮;
- `BrButton`:主按钮/弱按钮;
- `BannerCard`:提示、空态、推荐横幅;
- `QuickEntryGrid` / `EntryCard`:我的页、设置页、功能入口;
- `MiniPlayerBar`:后续 TTS 播放条。
...
现在这些位置的形状、间距、颜色都可以通过 `ui/design` 统一调整。

### 2. A2 替换旧 UI

**结论:设计系统要接管旧页面,否则它只是“旁边多了一套新代码”。**

本轮继续新增:

- `BrTextField`:普通输入、密码输入、笔记输入统一入口;
- `InfoCard`:详情区、笔记面板等内容卡片统一入口;
- `BrActionDock` / `BrDockAction`:阅读选中文字后的黑色浮动工具条;
- `AiPromptChip` / `AiInputBar`:AI 临时窗口里的推荐问题和输入栏。

已迁移:

- 书籍详情页的简介卡、AI 阅读卡、开始阅读/加入书架按钮;
- 书籍列表、章节列表、书架书籍行的卡片形状;
- 我的页账号/密码输入框;
- 书架继续阅读入口,改用 `MiniPlayerBar`;
- 阅读器选中文字后的写想法面板、浮动操作条;
- AI 问书临时窗口的推荐问题、标签按钮、底部输入栏。

这一刀的价值不是“换了几个圆角”,而是把修改入口收敛了。以后要改输入框、按钮、AI 底部栏、选中文字工具条,优先改 `ui/design`,不再逐页翻旧代码。
